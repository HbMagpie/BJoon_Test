...
public class BoardMgr {
	...
	public Vector<BoardBean> getBoardList(String keyField, String keyWord,
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setCodeC(rs.getString("codeC"));
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert tblBoard(name,code,codeC,content,subject,ref,pos,depth,regdate,pass,count,ip,filename,filesize)";
			sql += "values(?, ?, ?, ?, ?, ?, 0, 0, now(), ?, 0, ?, ?, ?)";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, multi.getParameter("name"));
			pstmt.setString(2, multi.getParameter("code"));
			pstmt.setString(3, multi.getParameter("codeC"));
	...
	public void updateBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblBoard set name=?,subject=?,code=?,codeC=? where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, bean.getName());
			pstmt.setString(2, bean.getSubject());
			pstmt.setString(3, bean.getCode());
			pstmt.setString(4, bean.getCodeC());
